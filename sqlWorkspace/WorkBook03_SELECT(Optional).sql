...
SELECT STUDENT_NAME AS "학생 이름", STUDENT_NO AS "학번", STUDENT_ADDRESS AS "주소지"
FROM TB_STUDENT
...
SELECT G.STUDENT_NO, STUDENT_NAME, TO_CHAR(POINT, 'FM9.00')
FROM TB_GRADE G
JOIN TB_STUDENT S ON (G.STUDENT_NO = S.STUDENT_NO)
WHERE CLASS_NO = 'C3118100' AND TERM_NO = '200402'
ORDER BY POINT DESC, G.STUDENT_NO ASC
;

-- 6번
-- 학생 번호, 학생 이름, 학과 이름을 학생 이름으로 오름차순 정렬하여 출력하는 SQL문을 작성하시오.
SELECT STUDENT_NO, STUDENT_NAME, D.DEPARTMENT_NO
FROM TB_STUDENT S
...
SELECT CLASS_NAME, ROUND(AVG(POINT),1) AS "과목 별 평점"
FROM TB_CLASS C
...
SELECT STUDENT_NAME, S.STUDENT_NO
FROM TB_STUDENT S
JOIN TB_GRADE G ON (S.STUDENT_NO = G.STUDENT_NO)
JOIN TB_DEPARTMENT D ON (S.DEPARTMENT_NO = D.DEPARTMENT_NO)
WHERE DEPARTMENT_NAME = '국어국문학과'
GROUP BY STUDENT_NAME, S.STUDENT_NO
HAVING SUM(POINT) = (SELECT MAX(SUM(POINT))
                     FROM TB_STUDENT
...
SELECT DEPARTMENT_NAME, CATEGORY AS "계열 학과명", ROUND(AVG(POINT),1) AS "전공 평점"
