...
ORDER BY PROFESSOR_SSN DESC, PROFESSOR_NAME ASC
;
...
SELECT 
    CASE
        ...
        WHEN PROFESSOR_NAME LIKE '____'   THEN SUBSTR(PROFESSOR_NAME,2,3)
    END AS "이름"
FROM TB_PROFESSOR
;
...
SELECT STUDENT_NAME AS "동명이인", COUNT(STUDENT_NAME) AS "동명이인 수"
FROM TB_STUDENT
...
SELECT TERM_NO, ROUND(AVG(POINT), 1) AS "년도, 학기 별 평점"
FROM TB_GRADE
WHERE STUDENT_NO = 'A112113'
GROUP BY TERM_NO
ORDER BY TERM_NO
...
SELECT SUBSTR(TERM_NO,1,4), ROUND(AVG(POINT), 1) AS "년도 별 누적 평점"
FROM TB_GRADE
WHERE STUDENT_NO = 'A112113'
GROUP BY SUBSTR(TERM_NO,1,4)
;

SELECT ROUND(AVG(POINT),1) AS "총 평점"
FROM TB_GRADE
WHERE STUDENT_NO = 'A112113'
;
