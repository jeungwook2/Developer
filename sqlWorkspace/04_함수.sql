...
SELECT EMP_NAME, LPAD(SALARY*12, 15)
FROM EMPLOYEE
...
SELECT EMP_NAME, TO_CHAR(SALARY, 'ㄷ,999,999')
FROM EMPLOYEE
ORDER BY SALARY;
...
SELECT
    ...
    , EMP_NO AS 주민번호
    , CASE
        WHEN SUBSTR(EMP_NO, 1, 2) BETWEEN 80 AND 99 THEN 'MZ'
        END AS 세대
FROM EMPLOYEE
;

-- 급여에 따라 초급,중급,고급 구분하여 
-- 사원명, 급여, 구분 출력하기
-- 300만원 미만 : 초급
...
SELECT 
    ...
    , CASE
        ...
        WHEN SALARY BETWEEN 5000000 AND 7999999 THEN '고급'
        ELSE '특급'
        END AS 구분
FROM EMPLOYEE
ORDER BY SALARY DESC
...
-- 부서코드가 D1인 사원들의 평균 급여 조회
-- 소수점 아래는 버리고 출력
SELECT '평균 급여 : ' || FLOOR(AVG(SALARY)) || '원' AS ZZZ
FROM EMPLOYEE
WHERE DEPT_CODE = 'D1'
;
...
-- 전체 사원 중, 급여가 가장 많은 사원의 이름, 급여 조회
SELECT MAX(SALARY) AS 급여1등
FROM EMPLOYEE
;
...
-- 남자 사원 수 조회
SELECT COUNT(NVL(PHONE,0)) AS 남자사원
FROM EMPLOYEE
